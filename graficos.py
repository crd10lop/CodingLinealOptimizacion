...
from simplex import nombres_variables


def expresion_objetivo(c):
    c = np.asarray(c, dtype=float)
    nombres = nombres_variables(len(c))
    partes = []
    for coef, nom in zip(c, nombres):
        coef = float(coef)
        if coef == 0:
            continue
        a = abs(coef)
        coef_str = '' if abs(a - 1) < 1e-12 else f'{a:g}'
        sep = '·' if coef_str else ''
        term = f'{coef_str}{sep}{nom}'
        if not partes:
            partes.append(('-' if coef < 0 else '') + term)
        else:
            partes.append((' - ' if coef < 0 else ' + ') + term)
    return ''.join(partes) if partes else '0'


def extremos_objetivo(c, A, b, signos):
    c = np.asarray(c, dtype=float)
    n = len(c)
    if n not in (2, 3):
        return None
    try:
        V = vertices_factibles(A, b, signos, n)
    except Exception:
        return None
    if len(V) == 0:
        return None
    ...
    return {
        'z_max': float(z[i_max]), 'x_max': V[i_max],
        'z_min': float(z[i_min]), 'x_min': V[i_min],
    }
...
def grafico_region_2d(c, A, b, signos, x_opt=None, z_opt=None, es_max=True):
    c = np.asarray(c, dtype=float)
    A = np.asarray(A, dtype=float)
    b = np.asarray(b, dtype=float)
    nombres = nombres_variables(2)
    ...
    if x_opt is not None and len(x_opt) >= 2:
        fig.add_trace(go.Scatter(
            x=[x_opt[0]], y=[x_opt[1]],
            mode='markers',
            marker=dict(size=18, color='crimson', symbol='diamond', line=dict(color='black', width=2)),
            name=f'Optimo ({nombres[0]}={x_opt[0]:.2f}, {nombres[1]}={x_opt[1]:.2f})'
        ))

    expr = expresion_objetivo(c)
    if z_opt is not None:
        tipo = 'maximo' if es_max else 'minimo'
        titulo = f'Region factible   |   Z = {expr}   |   Z {tipo} = {z_opt:.2f}'
    else:
        titulo = f'Region factible   |   Z = {expr}'
    fig.update_layout(
        xaxis=dict(title=nombres[0], range=[0, max_x], gridcolor='lightgray'),
        yaxis=dict(title=nombres[1], range=[0, max_y], gridcolor='lightgray'),
        title=titulo,
        plot_bgcolor='white',
        height=600,
        showlegend=True
...
def grafico_objetivo_3d(c, A, b, signos, x_opt, z_opt):
    try:
        ...
        fig.update_layout(
            scene=dict(
                xaxis_title=nombres[0],
                yaxis_title=nombres[1],
                zaxis_title='Z',
                camera=dict(eye=dict(x=1.6, y=-1.6, z=1.2))
            ),
            title=f'Funcion objetivo   Z = {expresion_objetivo(c)}',
            height=650
        )
        return fig
    except Exception:
    ...
    try:
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(size=6, color=z_vals, colorscale='Viridis',
                        showscale=True, colorbar=dict(title='Objetivo'),
        ...
        fig.update_layout(
            scene=dict(
                xaxis_title=nombres[0],
                yaxis_title=nombres[1],
                zaxis_title=nombres[2],
                camera=dict(eye=dict(x=1.6, y=-1.6, z=1.2))
            ),
            title=f'Region factible 3D   |   Z = {expresion_objetivo(c)}',
            height=650
        )
        return fig
    except Exception:
...
def grafico_exploracion_3d(todas_eval, c, x_opt, z_opt, n_vars):
    try:
        ...
        if n_vars == 2:
            ...
            fig.update_layout(scene=dict(xaxis_title=nombres[0], yaxis_title=nombres[1], zaxis_title='Z'))
            ...
            fig.update_layout(scene=dict(xaxis_title=nombres[0], yaxis_title=nombres[1], zaxis_title=nombres[2]))
